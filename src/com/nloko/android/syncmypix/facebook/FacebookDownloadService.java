...
public class FacebookDownloadService extends Service {
	...
	private class MainHandler extends Handler
	{
		...
		private final Runnable showDownloadError = new Runnable () {
			public void run() {
				handleError(R.string.downloadservice_downloaderror);
			}
		};
		
		private final Runnable showFatalSyncError = new Runnable () {
			public void run() {
				handleError(R.string.downloadservice_fatalsyncerror);
			}
		};
		
		private final Runnable showCancelError = new Runnable () {
			public void run() {
				handleError(R.string.downloadservice_canceled);
			}
		};
		
		private final Runnable showSessionError = new Runnable () {
			public void run() {
				handleError(R.string.downloadservice_sessionerror);
			}
		};
	...
	private class FacebookLogin extends Thread
	{
		...
		public void run()
		{
			...
			try {
				...
				if (client == null) {
					mainHandler.post(mainHandler.showSessionError);
				}
				else {
					mainHandler.post(mainHandler.showDownloadError);
				}
    ...
    private class SyncTask extends AsyncTask <List<SocialNetworkUser>, Integer, Long>
    {
		...
		protected Long doInBackground(List<SocialNetworkUser>... users) {
			...
			try {
				...
				for (SocialNetworkUser user : userList) {
					...
					if (cancel) {
						mainHandler.post(mainHandler.showCancelError);
						break;
					}
				}
				...
				Log.e(TAG, android.util.Log.getStackTraceString(ex));
				mainHandler.post(mainHandler.showFatalSyncError);
			}
			finally {
				mainHandler.post(mainHandler.resetExecuting);
			}
		...
		protected void onPostExecute(Long result) {
			if (result > 0 && !cancel) {
				cancelNotification(R.string.downloadservice_started, R.string.downloadservice_stopped);
				showNotification(R.string.downloadservice_stopped, android.R.drawable.stat_sys_download_done, true);
			}
			else {
				cancelNotification(R.string.downloadservice_started);
			}
	...
	public void onStart(Intent intent, int startId) {
		...
		notifyManager.cancel(R.string.downloadservice_stopped);
		
		showNotification(R.string.downloadservice_started, android.R.drawable.stat_sys_download);
		launchProgress();
    ...
    {
    	cancelNotification(R.string.downloadservice_started, msg);
    }
